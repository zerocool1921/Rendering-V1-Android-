...
def detect_ffmpeg_hwaccel():
    ...
    try:
        ...
        if mode in ["AUTO", "MEDIACODEC"] and IS_TERMUX and test_encoder_available("h264_mediacodec", log_reason=True):
            ...
            # PERBAIKAN: sebelumnya [] -> semua DECODE video (input utama,
            # sumber overlay sebelum di-bake, dst) selalu jatuh ke software
            # walau ENCODE sudah hardware (h264_mediacodec). Sudah dites &
            # dipastikan device mendukung "-hwaccel mediacodec" untuk decode
            # (termasuk dipakai bareng libavfilter/-filter_complex), jadi
            # sekarang diisi supaya get_gpu_input_hwaccel_flags() &
            # auto-inject di utils.run_ffmpeg_silent/run_ffmpeg_with_progress
            # ikut memakainya -> decode JUGA lewat MediaCodec, bukan cuma encode.
            STATE["ffmpeg_hw_input_args"] = ["-hwaccel", "mediacodec"]
            STATE["ffmpeg_hw_extra_args"] = [
                "-c:v", "h264_mediacodec",
                "-b:v", "6M",
                "-pix_fmt", "nv12"
